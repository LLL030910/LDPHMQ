...
void release_point_set(point_set_t* &point_set_v, bool clear)
{
	...
	free(point_set_v);
	point_set_v = NULL;
	// std::cout << "success delte memory..." << std::endl;
}
...
point_set_t* PointVectorToPointSet(const vector<Point>& points){
	if (points.empty()) {
		return nullptr;
	}

	// 使用现有的alloc_point_set函数
	point_set_t* point_set = alloc_point_set(points.size());

	for (size_t i = 0; i < points.size(); i++) {
		const Point& src_point = points[i];
		int dim = src_point.get_dimension();

		// 使用现有的alloc_point函数
		point_t* dest_point = alloc_point(dim);
		dest_point->id = src_point.getId();

		// 设置坐标
		vector<double> coords = src_point.get_coordinates();
		for (int j = 0; j < dim; j++) {
			dest_point->coord[j] = coords[j];
		}

		point_set->points[i] = dest_point;
	}
