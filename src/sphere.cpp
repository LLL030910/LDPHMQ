...
// The complete Sphere algorithm -----original
...
point_set_t* sphereWSImpLP2(point_set_t* point_set, int k)
{
	...
	{
		b_value[j] = 0;
		point_t* candidate = NULL;

		// Find max point in this dimension
		for (int i = 0; i < point_set->numberOfPoints; i++)
		{
			if (point_set->points[i]->coord[j] > b_value[j])
			{
				b_value[j] = point_set->points[i]->coord[j];
				candidate = point_set->points[i];
			}
		}
		...
		if (candidate != NULL)
		{
			...
			for (int p = 0; p < count; p++)
			{
				if (candidate->id == result->points[p]->id)
				{
					isNew = false;
					break;
				}
			...
			if (isNew)
			{
				result->points[count++] = candidate;
			}
			// If duplicate, ignore this point
		}
	}

	// 删除这行：count = dim;  // 错误的赋值！

	int initial_boundary_count = count;  // 保存实际边界点数量
